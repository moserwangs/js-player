...
`1.`then方法返回一个新的promise对象（不能直接返回this，因为promise的状态一旦成功或失败无法再改变）

`2.`then方法中onFulfilled或onRejected方法执行有三种情况：①返回普通值 ②执行抛错 ③返回一个新的promise对象

### 实现情况 ①返回普通值 ②执行抛错

`1.`第一个then方法中onFulfilled或onRejected方法执行有返回值或默认返回undefined，下一个then方法都会执行onFulfilled方法

`2.`第一个then方法中onFulfilled或onRejected方法执行抛错，下一个then方法都会执行onRejected方法
